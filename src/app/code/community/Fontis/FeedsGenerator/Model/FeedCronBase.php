...
abstract class Fontis_FeedsGenerator_Model_FeedCronBase
{
    ...
    protected function getPath()
    {
        $outputPath = Mage::getStoreConfig(
            'fontis_feedsgenerator/'.$this->_configPath.'/output',
            $this->info('store_id')
        );
    ...
    public function generateFeed()
    {
        ...
        foreach ($stores as $this->_store) {
            if (!Mage::getStoreConfig('fontis_feedsgenerator/'.$this->_configPath.'/active', $this->_store->getId())) {
    ...
    private function batchProcessStore($catId = null)
    {
        ...
        $includeAll = Mage::getStoreConfig(
            'fontis_feedsgenerator/'.$this->_configPath.'/include_all_products',
            $this->info('store_id')
        );
        $overrideAttributeCode = Mage::getStoreConfig(
            'fontis_feedsgenerator/'.$this->_configPath.'/custom_filter_attribute',
            $this->info('store_id')
        );
    ...
    private function collectDataForAccumulatedEntities()
    {
        ...
        $stdErr = Mage::getBaseDir('log') . '/fontis_feedsgenerator_subprocess.txt';
    ...
    protected function getConfig($key)
    {
        ...
        return Mage::getStoreConfig('fontis_feedsgenerator/' . $this->_configPath . '/' . $key, $storeId);
    }
