...
class Fontis_FeedsGenerator_Model_Child
{
    /**
     * @var array
     */
    ...
    function log($msg)
    {
        Mage::log("Child: " . $msg, null, 'fontis_feedsgenerator_child.log');
    }
        ...
        foreach ($attributes as $attribute) {
            if ($attribute->type == 'constant') {
                ...
                switch ($attribute->magento) {
                    ...
                    case 'currency':
                        $value = Mage::getStoreConfig('fontis_feedsgenerator/' . $this->config->config_path . '/currency');
                        break;
                    ...
                    case 'category_last':
                        ...
                        if ($attribute->magento == 'category_last') {
                            if (count($value) > 0) {
                                ...
                                $value = utf8_encode(Mage::getStoreConfig('fontis_feedsgenerator/'. $this->config->config_path .'/defaultcategory'));
                            }
                        }
                        break;
